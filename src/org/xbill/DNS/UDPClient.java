...
public
UDPClient(DatagramChannel channel, long endTime) throws IOException {
	super(channel, endTime);
}
...
throws IOException
{
    Log.d("XBILL", "LOCAL " + local);
    Log.d("XBILL", "REMOTE " + remote);
    ...
    DatagramPacket result = new DatagramPacket(new byte[max], max);
    socket.receive(result);
    byte[] res = new byte[result.getLength()];
    System.arraycopy(result.getData(), 0, res, 0, res.length);
    return res;
}
